...
export class B64Service {
  ...
  alignmentToSmil(alignment: any, text_path: string, audio_path: string) {
    ...
    let middle = alignment.filter((x: any) => x['word'] !== '<sil>').map((x: any) =>
      `<par id="par-${x['word']}">
     <text src="${text_path}#${x['word']}"/>
    <audio src="${audio_path}" clipBegin="${x['start']}" clipEnd="${x['end']}"/>
    </par>`)
    let parser = new DOMParser()
    let xml_doc = parser.parseFromString(topLine + middle.join("") + bottomLine, "application/xml")
    return `data:application/xml;base64,${this.xmlStringToB64(topLine + middle + bottomLine)}`
